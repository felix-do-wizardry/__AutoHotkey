...
while ( A_Index<=100 ) {
	sleep 20
	PixelGetColor, color, pX[8] + 12 , pY[8] + 25 , RGB
	if ( color >= 0x2A2A2A )
		break
...
while (A_Index<=9, i:=A_Index-1)
{
	...
	if ( color >= 0x2a0000 && color < 0x300000 )
		blockV[i, j] := 0
	else if ( color >= 0x300000 && color < 0xd00000 )
	{
		code := 0
		value := 0
...
;------ Command 012
i := 0
while ( fulfilledCount < numberCount )
{
...
;------ Command 3 [OFF]
if (1) {
while ( A_Index<=9, y:=A_Index-1)
while ( A_Index<=9, x:=A_Index-1)
{
	if ( blockV[x, y] == 0 )
	{
		blockV[x, y] := -1
...
if ( autoRun > 0 )
{
	...
	sleep 100
	PixelGetColor, color, pX[8] + 12 , pY[8] + 25 , RGB
	PixelGetColor, color9, 777 , 377 , RGB
	if ( color == 0x222222 && color9 == 0xD55336)
		break
